...
import kotlinx.serialization.cbor.internal.CborBooleanSerializer
import kotlinx.serialization.cbor.internal.CborByteStringSerializer
import kotlinx.serialization.cbor.internal.CborFloatSerializer
import kotlinx.serialization.cbor.internal.CborIntSerializer
import kotlinx.serialization.cbor.internal.CborNullSerializer
import kotlinx.serialization.cbor.internal.CborPrimitiveSerializer
import kotlinx.serialization.cbor.internal.CborStringSerializer
import kotlinx.serialization.cbor.internal.CborUndefinedSerializer
...
/**
 * Returns a Kotlin primitive representing this CBOR primitive.
 *
 * - [CborNull] maps to `null`
 * - [CborBoolean], [CborString], [CborFloat], [CborByteString] map to their respective Kotlin types
 * - [CborInteger] maps to [Long] when possible, otherwise (for positive numbers) to [ULong]
 *
 * Throws for [CborUndefined] and for negative integers that cannot be represented as [Long].
 */
@ExperimentalSerializationApi
public fun CborPrimitive.toKotlinPrimitive(): Any? = when (this) {
    is CborNull -> null
    is CborBoolean -> value
    is CborString -> value
    is CborFloat -> value
    is CborByteString -> value
    is CborInteger -> longOrNull ?: if (isPositive) value else throw ArithmeticException("$this cannot be represented as Long")
    is CborUndefined -> throw IllegalStateException("CBOR undefined value has no Kotlin primitive representation: $this")
}
